...
val t3 = new Task("Order wine for Friday", high, todo)
...
val t7 = new Task("Book accomodation for talk", medium, done)
...
def highPriorityTasks: Task => Boolean = (task: Task) => task.priority == high
...
def tasksDone = (task: Task) => task.status == done
...
println("------------------------------")
getTasksMatching(tasksDone) foreach println