...
object ImperativeProgramming extends App {
  ...
  function application is evaluated by replacing it with the function’s right-hand side, and, at the same time,
   by replacing the formal parameters by the actual arguments.
  ...
  */
  ...
  // Now rename all occurences of y with x in this sequence. We obtain:
  x deposit 30
  ...
  */
  ...
  Example:
  for (i <- until 3; j <- "abc") println(s"$i $j")
  translates to:
  */
  ...
  */
  ...
  println(factorial(2)) // shouldBe 2
  println(factorial(3)) // shouldBe 6
  println(factorial(4)) // shouldBe 24
  println(factorial(5)) // shouldBe 120
