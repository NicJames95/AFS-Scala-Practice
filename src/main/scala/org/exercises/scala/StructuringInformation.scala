...
object StructuringInformation extends App {
  ...
  First, let’s focus on notes. Suppose that, in our program, we are interested in the following properties of notes:
  their name (A, B, C, etc.), their duration (whole, half, quarter, etc.) and their octave number.

  In summary, our note model aggregates several data (name, duration and octave). We express this in Scala by using
  ...
  */
  ...
  So, we want to introduce the concept of symbol, as something that can be embodied by a fixed set of alternatives:
  a note or rest. We can express this in Scala using a sealed trait definition:
  */
  sealed trait Symbol

  case class Notes(name: String, duration: String, octave: Int) extends Symbol

  case class Rest(duration: String) extends Symbol

  // A sealed trait definition introduces a new type (here, Symbol), but no constructor for it. Constructors are defined
  // by alternatives that extend the sealed trait:
  val symbol1: Symbol = Notes("C", "Quarter", 3)
  val symbol2: Symbol = Rest("Whole")

  /*
  PATTERN MATCHING
  Since the Symbol type has no members, we can not do anything useful when we manipulate one. We need a way to distinguish
  between the different cases of symbols. Pattern matching allows us to do so:
  */
  def symbolDuration(symbol: Symbol): String =
    symbol match {
      case Notes(name, duration, octave) => duration
      case Rest(duration) => duration
    }
  /*
  The above match expression first checks if the given Symbol is a Note, and if it is the case it extracts its fields
  (name, duration and octave) and evaluates the expression at the right of the arrow. Otherwise, it checks if the given
  Symbol is a Rest, and if it is the case it extracts its duration field, and evaluates the expression at the right of the arrow.
  When we write case Rest(duration) => …, we say that Rest(…) is a constructor pattern: it matches all the values of
  type Rest that have been constructed with arguments matching the pattern duration.
  ...
  */
  ...
  It is generally a bad idea to work with a model that makes it possible to reach invalid states. In our case,
  we want to restrict the space of the possible note names and durations to a set of fixed alternatives.

  In the case of note names, the alternatives are either A, B, C, D, E, F or G. We can express the fact that note names
  are a fixed set of alternatives by using a sealed trait, but in contrast to the previous example alternatives are
  not case classes because they aggregate no information:
  */
  ...
  you will express it with a case class:

  “A note has a name, a duration and an octave number.”

  case class Note(name: String, duration: String, octave: Int) extends Symbol
  EXERCISE
  Consider the following algebraic data type that models note durations. Complete the implementation
  of the function fractionOfWhole, which takes a duration as a parameter and returns the corresponding fraction of the Whole duration.
  */
