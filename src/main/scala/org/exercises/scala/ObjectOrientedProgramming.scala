...
object ObjectOrientedProgramming extends App {
  ...
  */
  class Rationale(x: Int, y: Int) {
    ...
    def less(that: Rationale) =
      numer * that.denom < that.numer * denom

    def max(that: Rationale) =
  ...
  */
  ...
  /*
  EXERCISE
  The following Reducer abstract class defines how to reduce a list of values into a single value by
  starting with an initial value and combining it with each element of the list:
   */
  ...
  println(Product.reduce(nums)) // shouldBe 24
  println(Sum.reduce(nums)) // shouldBe 10
